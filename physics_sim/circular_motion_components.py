                ...
                const GraphPanel = ({ title, xFunc, yFunc, xVal, yVal, xLabel, yLabel, colorX, colorY, scale = 1, yMax="1.0" }) => (
                    <div className="bg-slate-50/50 p-4 rounded-3xl border border-slate-100 space-y-2">
                        ...
                        <div className="relative h-[120px] w-full bg-white rounded-xl border border-slate-100 overflow-hidden shadow-inner flex">
                            {/* Y축 범례 및 값 */}
                            <div className="w-8 h-full border-r border-slate-50 flex flex-col justify-between py-1 text-[9px] font-bold text-slate-400 text-center select-none bg-slate-50/30">
                                <span>{yMax}</span>
                                <span>0</span>
                                <span>-{yMax}</span>
                            </div>
                            ...
                            <div className="flex-1 relative overflow-hidden">
                                ...
                                {/* X축 시간 범례 */}
                                <div className="absolute bottom-0 left-0 w-full px-2 flex justify-between text-[9px] font-black text-slate-300 pointer-events-none">
                                    <span>0s</span>
                                    <span>시간 (t)</span>
                                    <span>{maxTime}s</span>
                                </div>
                        ...
                        <div className="w-full max-w-7xl rounded-[32px] shadow-[0_20px_40px_-10px_rgba(0,0,0,0.15)] border border-slate-200 overflow-hidden bg-white">
                            ...
                            <div className="flex flex-col lg:flex-row divide-x divide-slate-100 min-h-[640px]">
                                <div className="lg:w-[450px] bg-slate-50 flex flex-col items-center justify-center p-8 relative">
                                    <div className="absolute top-6 left-6 flex flex-col gap-2 z-10 w-[120px]">
                                        <button onClick={()=>setShowPos(!showPos)} className={`w-full py-1.5 rounded-lg text-[10px] font-black border-2 transition-all ${showPos ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white border-slate-200 text-slate-400'}`}>위치(r) 벡터</button>
                                        <button onClick={()=>setShowVel(!showVel)} className={`w-full py-1.5 rounded-lg text-[10px] font-black border-2 transition-all ${showVel ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-white border-slate-200 text-slate-400'}`}>속도(v) 벡터</button>
                                        <button onClick={()=>setShowAcc(!showAcc)} className={`w-full py-1.5 rounded-lg text-[10px] font-black border-2 transition-all ${showAcc ? 'bg-amber-500 border-amber-500 text-white' : 'bg-white border-slate-200 text-slate-400'}`}>가속도(a) 벡터</button>
                                    </div>
                                    ...
                                    <svg viewBox="0 0 400 400" className="w-full h-full max-w-[340px] drop-shadow-2xl">
                                        ...
                                        {showPos && (
                                            <g>
                                                <line x1="200" y1="200" x2={200 + pos.x * 120} y2={200 - pos.y * 120} stroke="#3b82f6" strokeWidth="2" />
