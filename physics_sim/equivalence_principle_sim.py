...
function drawPhase2(ctx,W,H,accel,t,running){
    ...
    txt(ctx,W*0.04+38,qY+17,'관성력과 중력을 구별할 수 없다면, 중력 하에서도 빛이 휘어야 하는 것 아닐까?','rgba(255,220,120,0.92)',11,'left','700');
...
function drawPhase3(ctx,W,H,accel,t,running){
    ...
    const massStr=600*(accel/9.8);   /* 시공간 곡률 강도 - 더 강하게 */
...
const QNA_ITEMS = [
    ...
    {q:'관성력과 중력을 구별할 수 없다면, 중력 하에서도 빛이 휘어야 하는 것 아닐까?',a:'네, 그렇습니다! 가속 좌표계에서 빛이 아래로 휘어보이는 것은 관성력 때문입니다. 관성력과 중력이 구별 불가(등가원리)라면, 중력장에서도 빛이 반드시 휘어야 합니다 — 같은 원인(가속도/중력)에 같은 결과(빛의 휨)가 나타나야 하기 때문입니다. 즉, 가속 좌표계와 중력장 모두에서 빛은 아래로 휘어보입니다.'},
